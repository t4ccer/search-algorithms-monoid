...
nextSearchStateM better mk_key nextM old = do
  ...
  where
    mk_search_state new_paths (new_current, remaining_queue) =
        SearchState
            { current = new_current
    ...
    update_queue_paths (old_queue, old_paths) st =
        if mk_key st `Set.member` visited old
            ...
            else case Map.lookup (mk_key st) old_paths of
                Just old_path ->
...
generalizedSearchM empty mk_key better nextM foundM initial = do
    let initial_state =
            SearchState
                initial
                empty
                (Set.singleton $ mk_key initial)
                (Map.singleton (mk_key initial) [])
    end_May <-
        findIterateM
            (nextSearchStateM better mk_key nextM)
            (foundM . current)
            initial_state
